...
if [ -e fio.txt ]; then
  ...
  ALL=`wc -l fio.txt | cut -f1 -d' '`
  REPEAT=$(($ALL - $READ - $WRITE))
  TESTS=$(($READ + $WRITE))
  REPEAT=$(($REPEAT/$TESTS))
  SPLIT=0
  if [ $READ -eq 1 ]; then
    SPLIT=$(($SPLIT + $REPEAT + 1))  
